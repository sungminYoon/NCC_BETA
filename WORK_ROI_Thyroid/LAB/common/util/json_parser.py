...
def depth_one(info_list):
    ...
    for info in info_list:
        ...
        # 좌표 dict key = 좌표 그룹 번호, value = 좌표 리스트 넣어 줍니다.
...
def mask_position_save_text(path, data_list):
    ...
    for image_info in data_list:
        ...
        # 이미지 넘버를 contour 그룹에 저장
        contour_group.append(image_number)

    # 결과를 파일로 저장 합니다.
    json_file_save(path, contour_group, group_name)


# json 파일로 저장 합니다.
def json_file_save(path, json_data, group_name):
    ...
    file_path = f'{folder_path}{group_name}_{file_name}{file_extension}'

    # 파일 경로, 이름을 가지고 json type 저장 합니다.
