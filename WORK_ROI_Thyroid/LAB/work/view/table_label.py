...
from LAB.common.util import notice, file_manager

SAVE_JSON_ALIAS = 'label'
BOX_SIZE_WIDTH = 512
BOX_SIZE_HEIGHT = 150
CELL_SIZE_WIDTH = 135
CELL_SIZE_HEIGHT = 30
...
class TableLabel:
    ...
    def click_save_label(self, parameter):
        ...
        if self.call_index() is None:
            notice.message('Error', '좌측 테이블 image 선택해 주세요!')
            return 0
        ...
        # 활성 화된 img 경로를 가지고 옵니다.
        path = self.call_path()
        window = self.call_window()
        muscle = self.call_muscle()

        # json 그룹 속에 key 와 value 를 만듭니다.
        img = dict()
    ...
    def lineChanged(self, sender):
        ...
        # user 입력한 text data list 에 저장 합니다.
        if sender.text() is '':
            print('TableLabel: empty')
        else:
            self.user_input_list[number] = line_edit.text()

    # 자동 으로 해당 path json data 를 가지고 옵니다.
    def json_auto_read(self, img_folder):
        ...
        # json 파일의 첫번째 파일명 확인
        json_str = _json_list[0]
        chk_str = json_str.split('_')

        # 생성한 json 파일 인지 판정
        if len(_json_list) < 1 or chk_str[0] != SAVE_JSON_ALIAS:
    ...
    def json_parsing(self):
        ...
        try:
            # JSON 의 KEY 와 VALUE data 가지고 옵니다.
        ...
        self._handling(mouse_position_list,
                       user_select_image,
                       window, muscle,
                       path)

    # 뷰로 테이블 라벨의 정보를 보냅 니다.
    def _handling(self, mouse_position_list,
                  user_select_image,
                  window,
                  muscle,
                  path):

        print('TableLabel: _handling-> call_load_view')
