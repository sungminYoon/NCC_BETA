...
class Provider:
    ...
    info_list = None            # 이미지 정보 데이터 list 만듭니다.
    img_container = None        # Open cv 이미지 모음 입니다.
    img_level_list = None       # 이미지 LEVEL 변환 모음 입니다.
    img_group_level = None      # 이미지 update 사용 되는 리스트 입니다.
    img_group_threshold = None  # 이미지 광역 threshold 리스트
    ...
    def create(self, data_folder, extension):
        ...
        # container 에서 image_set 을 생성 합니다.
        self.img_container = container.set_cv_image(data_folder, file_list_img)

        # container 에서 image_set 파일 쓰기 시간을 줍니다.
        time.sleep(1)
        print('Provider: Open cv image data set 생성을 완료.')
    ...
    # 넘버와 이미지 가 들어간 GRAY 스케일 CV 이미지 update
    def data_update_level(self, window, muscle):
        ...
        notice.message('변환', '이미지 update 합니다.')
